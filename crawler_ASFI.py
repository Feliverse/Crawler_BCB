...
# === ESCANEO DE DOCUMENTOS CON ESTRUCTURA COMPATIBLE CON BCB ===
def escanear_documentos_y_estructurar(nivel1_macro, nombre_pagina, url_pagina, mapa_final):
    try:
        ...
        # Usamos "ESTADISTICAS" como raíz fija (igual que el modelo BCB)
        raiz = "ESTADISTICAS"
        ...
        for e in enlaces:
            ...
            # === Nivel 2: Categoría (usamos el nombre de la página) ===
            nivel2 = normalizar_nombre(nombre_pagina)

            # === Nivel 3 y 4: Fijos "DOCUMENTOS" y "VARIOS" (compatible con BCB) ===
            nivel3 = "DOCUMENTOS"
            nivel4 = "VARIOS"

            # === Nombre del archivo (sin carpetas intermedias) ===
            ...
            # Extraer fecha de la ruta (si existe) y asignarla
            fecha = extraer_fecha_de_ruta(path_decoded)

            # Construir el árbol exactamente como en BCB
            if nivel2 not in mapa_final[raiz]:
                mapa_final[raiz][nivel2] = {}
            if nivel3 not in mapa_final[raiz][nivel2]:
                mapa_final[raiz][nivel2][nivel3] = {}
            if nivel4 not in mapa_final[raiz][nivel2][nivel3]:
                mapa_final[raiz][nivel2][nivel3][nivel4] = {}

            if nombre_archivo not in mapa_final[raiz][nivel2][nivel3][nivel4]:
                mapa_final[raiz][nivel2][nivel3][nivel4][nombre_archivo] = {
...
if __name__ == "__main__":
    ...
    # Guardar el JSON en la carpeta Crawlers/
    import os
    os.makedirs("Crawlers", exist_ok=True)
    with open("Crawlers/mapa_global_asfi.json", "w", encoding="utf-8") as f:
        json.dump(mapa_final, f, ensure_ascii=False, indent=4)

    print("\n✅ ¡Listo! Mapa generado en Crawlers/mapa_global_asfi.json")
    total_docs = 0
