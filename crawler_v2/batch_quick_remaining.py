...
REQUEST_TIMEOUT = 10
CRAWLER_TIMEOUT = 300  # 5 minutos máximo por fuente

DELAY_SECONDS = 0.15
RANDOM_DELAY_MIN = 0.15
RANDOM_DELAY_MAX = 0.35

API_PAGINATION_MAX_PAGES = 5
MAX_OPENAPI_ENDPOINTS = 40
...
FULL_OUTPUT_DIR = OUTPUT_DIR / "quick_map"
LOG_DIR = OUTPUT_DIR / "quick_logs"
...
def save_results(results: list[dict], skipped: list[dict]) -> None:
    json_path = OUTPUT_DIR / "quick_resultados.json"
    csv_path = OUTPUT_DIR / "quick_resultados.csv"
    skipped_path = OUTPUT_DIR / "quick_omitidos.json"
...
def print_final_summary(results: list[dict], skipped: list[dict]) -> None:
    ...
    print()
    print("=" * 80)
    print("RESUMEN FINAL - DIAGNOSTICO RAPIDO")
    print("=" * 80)
    ...
    print()
    print(
        "MAPEADO = agotó la frontera dentro del límite rápido y sin errores."
    )
    print(
...
# ============================================================
# SALIDAS PROFUNDAS YA TERMINADAS
# ============================================================

DEEP_OUTPUT_DIR = OUTPUT_DIR / "full_map"


def deep_output_path(source: dict) -> Path:
    return DEEP_OUTPUT_DIR / (
        f"{int(source['fila_excel']):03d}_{slugify(source['fuente'])}.json"
    )


def has_completed_deep_output(source: dict) -> bool:
    path = deep_output_path(source)

    if not path.exists():
        return False

    metrics = inspect_output_json(path)
    return int(metrics.get("recursos_json", 0)) > 0
...
def main() -> None:
    ...
    print("=" * 80)
    print("BATCH RAPIDO - DIAGNOSTICO DE FUENTES PENDIENTES")
    print("=" * 80)
    ...
    print()
    print("Límites de diagnóstico rápido:")
    print(f"  max_pages = {MAX_PAGES}")
    print(f"  max_depth = {MAX_DEPTH}")
    print(f"  max_files = {MAX_FILES}")
    print("Objetivo: detectar rápido qué fuentes funcionan y cuáles requieren ajuste.")
    ...
    completed_deep = [
        ...
        if has_completed_deep_output(source)
    ]

    pending_sources = [
        source
        for source in sources
        if not has_completed_deep_output(source)
    ]

    print()
    print(f"Ya terminadas con JSON profundo no vacío: {len(completed_deep)}")
    print(f"Pendientes para diagnóstico rápido:      {len(pending_sources)}")
    print("Los JSON profundos existentes NO se borran ni se repiten.")

    if completed_deep:
        print()
        print("Omitidas por tener salida profunda:")
        for source in completed_deep:
            print(
                f"  - {source['fuente']} | "
                f"{deep_output_path(source).name}"
            )
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_map = {
            ...
            for position, source in enumerate(pending_sources, start=1)
        }
    ...
    print(
        f"Resumen JSON: "
        f"{OUTPUT_DIR / 'quick_resultados.json'}"
    )
    print(
        f"Resumen CSV : "
        f"{OUTPUT_DIR / 'quick_resultados.csv'}"
    )
